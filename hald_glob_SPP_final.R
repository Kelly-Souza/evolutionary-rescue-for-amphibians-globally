setwd("C:/Users/lets/Downloads/backup kelly/Lets/Desktop/FRONTIERS")
...
for(i in 1:ncol(PA)){
  ...
  setTxtProgressBar(pb2, i)
	...
	out.spp[i,11] <-mean(freq.rg.genet) #incluir_K
	out.spp[i,12] <-mean(freq.rg.plast) #incluir_K
	
}
head(out.spp)
colnames(out.spp) <-c("Range","sd.phy","sd.rg","Latitude", "Longitude","meanT0","meanT1","Haldane","P_Rescue","P_RescueP","Geo_freq","Geo_FreqR","SumExp_Range")
#write.table(out.spp,"Rescue_GLOBAL_RCP85_SN_weak")
#write.table(out.spp,"Rescue_GLOBAL_RCP85_SN_Medium")
#write.table(out.spp,"Rescue_GLOBAL_RCP85_SN_Strong")
...
plot(out.spp[,9],out.spp[,10],cex=0.5,pch=16,xlab="Probability of Rescue (Genetic)",ylab="Probability of rescue (Plasticity)")
abline(a=0,b=1,lwd=2,lty=2,col="black")

plot(out.spp[,9],out.spp[,10],cex=0.5,pch=16,xlab="Probability of Rescue (H > MSER)",ylab="Probabilit of rescue (H > MSER-P)")
abline(a=0,b=1,lwd=2,lty=2,col="black")
...
summary(lm((out.spp[,11]/out.spp[,13])~log(out.spp[,1])+out.spp[,2]+out.spp[,4]+out.spp[,5]))
plot((out.spp[,11]/out.spp[,13]),out.spp[,3])
...
zoor <-read.table("Rescue_GLOBAL_RCP85_SN_50_without_DISP_8.5_zooregions.txt", h=T)

head(zoor)
str(zoor)


summary(aov(zoor$Rescue ~ zoor$zooregions))

summary(aov(zoor$RescueP ~ zoor$zooregions))

boxplot(zoor$Rescue ~ zoor$zooregions)
boxplot(zoor$RescueP ~ zoor$zooregions)
boxplot(zoor$P_Rescue + zoor$Range ~ zoor$zooregions)
boxplot(zoor$P_RescueP + zoor$Range ~ zoor$zooregions)
